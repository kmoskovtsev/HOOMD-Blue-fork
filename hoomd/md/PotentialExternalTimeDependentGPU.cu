...
//! Evaluator for External Periodic Time Dependent potentials.
template cudaError_t gpu_cpef<EvaluatorExternalPeriodicTime>(const external_potential_args_t& external_potential_args, const typename EvaluatorExternalPeriodicTime::param_type *d_params, const unsigned int timestep, const typename EvaluatorExternalPeriodicTime::field_type *d_field);
