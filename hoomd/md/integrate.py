...
class custom_scatter2D(_integration_method):
    R""" NVE Integration with custom scattering; velocities are propagated via Velocity-Verlet, and elastic scattering events rotate velocity at random moments in time.
    ...
    :py:class:`custom_scatter2D` performs constant volume, constant energy simulations using the standard
    Velocity-Verlet method but also includes elastic scattering specified using `set_tables` method. The method
    is based on `nve` integrator and all nve's functionality is preserved.
    
    The scattering rates are specified using `set_tables(wk, Winv, vmin, vmax)` method. `wk` is a 1D array
    of scattering rates (inverse time units), specifying total scattering rates for velocities equidistantly
    spaced from `vmin` to `vmax`. If velocity of a particle in a simulation is beyond those limits, the limit
    values are used. `Winv` is a 2D array (`Nk` x `NW`) specifying angle distribution of scattering rates. 
    Row `i` of `Winv` must represent the inverse of cumulative distribution function `W(theta)`,
    sampled uniformly on [0,1].

    On each timestep for each particle, `custom_scatter2d` decides if the particle should scatter at this
    timestep. This is done randomly based on scattring rates specified in `wk`. If the particle is scattered,
    the angle is randomly generated according to the distribution provided in `Winv`. The correct distribution
    is generated by means of Inverse Transform Sampling. That is why the inverse cumulative distribution must
    be proviced in `Winv`.
    ...
    :py:class:`custom_scatter2d` is an integration method. It must be used with :py:class:`mode_standard`.

    A :py:class:`hoomd.compute.thermo` is automatically specified and associated with *group*.

    Examples::
    scatter = integrate.custom_scatter2d(group.all(), Nk=100, NW=100, seed=989)
    scatter.set_tables(wk, Winv, vmin, vmax)
    ...
    def __init__(self, group, Nk, NW, seed, limit=None, zero_force=False):
        ...
        if hoomd.context.current.system_definition.getNDimensions() != 2:
            hoomd.context.msg.error("System must be 2 dimensional\n");
            raise RuntimeError("Cannot use custom_scatter2d");
        # initialize base class
        _integration_method.__init__(self);
        ...
        if not hoomd.context.exec_conf.isCUDAEnabled():
            ...
            self.cpp_method = _md.TwoStepCustomScatter2DGPU(hoomd.context.current.system_definition, group.cpp_group, Nk, NW, seed);
