...
/*! \file PotentialExternalTimeDependent.h
    \brief Declares a class for computing an external force field that depends on time
*/
...
#ifndef __POTENTIAL_EXTERNAL_TIME_DEPENDENT_H__
#define __POTENTIAL_EXTERNAL_TIME_DEPENDENT_H__

//! Applys an external force to particles based on position
/*! \ingroup computes
*/
template<class evaluator>
class PotentialExternalTimeDependent: public ForceCompute
    {
    public:
        //! Constructs the compute
        PotentialExternalTimeDependent<evaluator>(std::shared_ptr<SystemDefinition> sysdef,
                                     const std::string& log_suffix="");
        virtual ~PotentialExternalTimeDependent<evaluator>();
...
*/
template<class evaluator>
PotentialExternalTimeDependent<evaluator>::PotentialExternalTimeDependent(std::shared_ptr<SystemDefinition> sysdef,
                         const std::string& log_suffix)
    : ForceCompute(sysdef)
    {
    ...
    m_pdata->getNumTypesChangeSignal().template connect<PotentialExternalTimeDependent<evaluator>, &PotentialExternalTimeDependent<evaluator>::slotNumTypesChange>(this);
    }

/*! Destructor
*/
template<class evaluator>
PotentialExternalTimeDependent<evaluator>::~PotentialExternalTimeDependent()
    {
    m_pdata->getNumTypesChangeSignal().template disconnect<PotentialExternalTimeDependent<evaluator>, &PotentialExternalTimeDependent<evaluator>::slotNumTypesChange>(this);
    }

/*! PotentialExternalTimeDependent provides
    - \c external_"name"_energy
*/
template<class evaluator>
std::vector< std::string > PotentialExternalTimeDependent<evaluator>::getProvidedLogQuantities()
    {
...
*/
template<class evaluator>
Scalar PotentialExternalTimeDependent<evaluator>::getLogValue(const std::string& quantity, unsigned int timestep)
    {
...
*/
template<class evaluator>
void PotentialExternalTimeDependent<evaluator>::computeForces(unsigned int timestep)
    {

    if (m_prof) m_prof->push("PotentialExternalTimeDependent");
   ...
   // there are enough other checks on the input data: but it doesn't hurt to be safe
    ...
    for (unsigned int idx = 0; idx < nparticles; idx++)
        ...
        evaluator eval(X, box, params, timestep, field);
...
*/
template<class evaluator>
void PotentialExternalTimeDependent<evaluator>::setParams(unsigned int type, param_type params)
    {
    if (type >= m_pdata->getNTypes())
        {
        this->m_exec_conf->msg->error() << "external.periodic_time: Trying to set external potential params for a non existant type! "
                                        << type << std::endl;
        throw std::runtime_error("Error setting parameters in PotentialExternalTimeDependent");
        }
...
template<class evaluator>
void PotentialExternalTimeDependent<evaluator>::setField(field_type field)
    {
...
*/
template < class T >
void export_PotentialExternalTimeDependent(pybind11::module& m, const std::string& name)
    {
